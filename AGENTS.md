...
- **QERDS requires a real provider — no in-process stub.** `QERDS_PROVIDER` + `QERDS_PROVIDER_URL` are required config; a QERDS-less full-app boot fails (deliberate — a missing provider used to silently send nothing). Dev points at the local Domibus AS4 bench, which is opt-in behind the `domibus` Compose profile, so `npm run dev` / `docker compose up` now needs `docker compose --profile domibus up` first (the dev override defaults `QERDS_*` to the bench, inbound mode `poll`). CI's `go test -race` is DB-free and does not boot the full app; unit tests use a test-local provider fake. Inbound delivery mode is `QERDS_INBOUND_MODE=push|poll|both` (`poll`/`both` run a background poll worker on `QERDS_POLL_INTERVAL`); push requires `QERDS_WEBHOOK_SECRET` or config load fails. See `.ai/features/qerds.md`.
