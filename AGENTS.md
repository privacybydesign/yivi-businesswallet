...
- **`internal/integration/setup_test.go` assembles its own router, not `cmd/api`'s.** It passes a hand-picked subset of feature handlers to `server.New`, so a new slice is invisible to the integration suite (its routes 404) until it is added there too. The two wirings are separate on purpose — the integration env stubs the verifier, issuer and holder — so adding a handler means editing both.
- **A new field on `user.User` has three query sites, none of which the compiler checks.** `pgx` scans into explicit destinations, so `session.Store.Lookup` (every authenticated request), `user.Store.FindByEmail` and `user.Store.GetByID` each need the column added by hand; miss one and the field is silently the zero value on that path only. `Create` is exempt — a brand-new user has no avatar, no membership, nothing derived.
- **A user-uploaded image is re-encoded before storage, not just sniffed.** `internal/useravatar.normalize` reads the header with `image.DecodeConfig` (rejecting a decompression bomb before allocating pixels), then decodes and re-encodes with the stdlib — which is what strips the photo's EXIF block, GPS coordinates included. That is why an avatar accepts only PNG and JPEG while the org logo (`themesettings`, sniffed and stored verbatim) also takes GIF, WebP and SVG. Both store bytes in Postgres, and serve them behind a `nosniff` + null-source-CSP + sandbox response; a per-user photo is additionally `Cache-Control: private`. The frontend downscales through a canvas first (`lib/avatar-image.ts`), so the byte and dimension caps are a guard against a hand-made request, not the path a person takes.
- **A new backend env var must also be listed in `compose.yaml`'s `backend` service.** Compose does not forward a root `.env` variable into a container unless the service declares it, so a variable `config.Load` reads but Compose omits looks configured in `.env` and silently keeps its built-in default. `backend/internal/config/compose_test.go` guards the PostGuard set and `MAIL_DEFAULT_LOCALE` that way — add a case there for each new variable.
