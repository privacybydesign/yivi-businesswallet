...
- **Per-org theming has a pre-paint cache to avoid a FOUC**: `lib/theme.ts` `resolveThemeTokens` is the single source of truth for the `--yb-*` overrides an org's colours map to; `applyOrgTheme` sets them at runtime and `cacheOrgTheme` mirrors the same map into `localStorage` under `ybw.theme.<slug>`. The inline script in `frontend/index.html` reads that key by the first path segment and applies it **before React hydrates**, so a full reload of an org page never flashes the default palette first. If you change the token set or the cache key, update the inline script in the same edit — it deliberately duplicates no derivation logic (it only replays the cached map) so the two cannot compute different palettes. The org logo shown in the sidebar and `ui/TopBar` comes from `ui/brand.ts` (`BrandProvider` in `root.tsx`); it is not part of the pre-paint cache (an `<img>`, not a CSS token).
- **The link token (`--yb-link`) is themed at runtime, not pre-paint.** `lib/theme.ts` `resolveLinkTheme` derives a brand-tinted link colour from the accent seed so links pick up the tenant brand instead of the default Yivi blue. Unlike the mode-agnostic seed fills, a link must stay legible on the surfaces it sits on, which differ between light and dark — so it derives a *pair* (accent darkened for light mode, lightened for dark mode, each cleared to WCAG-AA against the worst-case background). Because a media query can't live in an inline custom property, `applyOrgTheme` ships the pair as a `<style id="ybw-org-link-theme">` appended to `<head>` (it wins by source order and the browser switches modes natively). It is deliberately **not** in the pre-paint cache: a `<style>` appended pre-paint has no reliable source-order guarantee against Vite's CSS, and inline `documentElement` overrides can't carry the media query. The only cost is a brief default-blue link until React mounts — acceptable because link was never themed before, so there's no palette regression. `src/lib/theme.test.ts` pins the derivation at WCAG-AA on every surface in both modes.
- **Dark mode is OS-driven, pure CSS.** A single `@media (prefers-color-scheme: dark)` block in `frontend/src/index.css` overrides the neutral/surface/status/elevation `--yb-*` tokens with the design system's dark values (`Yivi Design System.zip` → `ui_kits/business/tokens.css`, `[data-theme="dark"]`); every utility resolves through those tokens so the whole shell flips at once. There is deliberately **no per-user light/dark toggle** — per-user theming is a stated non-goal (issue #64), so dark mode follows the OS preference only. The per-org brand seed is applied inline on the documentElement by the theme engine, so it wins over both the light and dark defaults and carries into dark mode unchanged (no mode-aware derivation needed — a primary/accent seed is a self-contained coloured fill with its own readable foreground). `src/index-css-dark.test.ts` parses the dark block and pins every rendered fg/bg pair at WCAG-AA, so a new or edited dark value can't silently regress contrast. NB: `--yb-muted` runs below AA in **both** themes (a pre-existing house-style choice for incidental captions), so it is not asserted.
