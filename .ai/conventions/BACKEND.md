...
- **The RBAC and consent *implementations* are not in `main`, only their design.** PR #128 (`RequirePermission(resource, action)`, the role→permission matrix, the extra roles) and PR #129 (the consent & approval layer) both report "Merged" on GitHub, but they were a stacked pair: #128 merged into `feat/rbac-model-design` and #129 into `feat/rbac-model-impl`, neither of which ever reached `main` after #117 landed the design doc. So `.ai/plans/rbac-model.md` is in `main` and the code it specifies is not — `git merge-base --is-ancestor <merge-sha> main` says `NO` for both. Before building on `RequirePermission`, the finer roles or the consent layer, grep for them; if the tenant access seam above still has only `Authorize` / `RequireOrgAdmin`, nothing of #128 is there. `internal/organization/mandate.go` is written against the seams that *are* in `main` for this reason.
...
- **Never edit a migration that has been applied anywhere.** Goose records version numbers, not file contents: an in-place rewrite is silently skipped on every database that already ran the original (dev volumes, staging) while fresh databases and CI get the new shape, so the schema drifts and only a real environment notices. `20260716150000_add_member_identity.sql` was rewritten in place and needed `20260914084007_repair_member_identity_verified_at.sql` to make staging whole again. Stack a new `ALTER` migration instead. A repair migration must be a no-op on both histories: `IF [NOT] EXISTS` for columns, and a `DO $$` block around any statement that names a column only one history has (PL/pgSQL parses a statement when it first runs, so a guarded statement never compiles where its column is missing). Prove it with a test like `internal/migrate/repair_integration_test.go`: `testdb.Bare` plus `migrate.UpTo` rebuilds the legacy shape, then `migrate.Up` must converge it.
...
- **`internal/integration`'s `env.login` cannot switch the authenticated caller.** Every call claims the one shared `disclosureToken`, and a session's idempotency key is that token, so `session.Store.Mint`'s `ON CONFLICT (idempotency_key) DO UPDATE` rotates the *first* user's session token and never touches its `user_id`. The `/claim` response body reports the second user (it is computed from the disclosure) while the cookie keeps resolving to the first, so the test silently keeps acting as whoever logged in first — it does not fail, it passes as the wrong caller. To swap callers, claim on a presentation id of that caller's own: `seedPresentation(t, pool, "presentation-"+email)` then `POST /api/v1/auth/session/<that id>/claim`. `internal/integration/mandate_test.go`'s `loginAs` is the helper.
