...
- **`go mod tidy` does not work in `backend/`, and that is pre-existing rather than something your change broke.** `tidy` resolves imports across *all* build configurations, so it reaches the `wsca`-gated import of `github.com/privacybydesign/wallet-provider/...` in `internal/eudiholder` — a private module that reaches the build through a Go workspace over a deploy-key clone (the `backend-build-wsca` job, `docker/development/wsca.work`) and is not in `go.mod` at all, so tidy exits 1 with `repository not found`. `go build` / `go vet` / `go test` skip that build config and are unaffected. Bump a dependency with `go get <mod>@<version>` — it updates `go.mod` and `go.sum` on its own — and never "fix" the result with tidy. If `go vet` then reports `missing go.sum entry` for a *new* transitive module of what you bumped, re-run `go get` on the direct dependency that pulled it in; that adds the entry without tidy.
