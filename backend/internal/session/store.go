...
func (s *Store) Lookup(ctx context.Context, rawToken string) (user.User, error) {
	...
	const q = `
		SELECT u.id, u.email, u.preferred_name, u.given_names, u.last_name, a.updated_at
		FROM sessions s
		JOIN users u ON u.id = s.user_id
		LEFT JOIN user_avatars a ON a.user_id = u.id
		WHERE s.token_hash = $1 AND s.expires_at > now()`
	var u user.User
	if err := s.db.QueryRow(ctx, q, hash[:]).Scan(&u.ID, &u.Email, &u.PreferredName, &u.GivenNames, &u.LastName, &u.AvatarUpdatedAt); err != nil {
