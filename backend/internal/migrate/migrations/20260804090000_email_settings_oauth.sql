...
--
-- This CHECK is where the forward compatibility above it stops. Every other layer
-- tolerates a mechanism it does not know — the frontend parses auth_mechanism as a
-- plain string rather than a closed enum, so an unknown stored value keeps the
-- settings screen up and stays selectable — but the constraint is a closed set. A
-- third mechanism added to mailer.KnownMechanism without a migration extending
-- this list passes email.parseSettingsRequest and then fails at the INSERT, which
-- putSettings reports as a 500 rather than the 400 the rest of the validation
-- produces. So: a new mechanism needs a migration here in the same change.
