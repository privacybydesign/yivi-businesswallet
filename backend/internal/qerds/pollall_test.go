...
func (m *multiOrgStore) ListAddresses(_ context.Context, orgID uuid.UUID) ([]Address, error) {
	owned := []Address{}
...
// The console's Poll reads the same shared WS-plugin queue as the sweep, so it
// needs the same finalRecipient attribution: a strayed message must land in the
// owning org's inbox, not the polling org's. It must also not be counted as
// received for the org that asked — it cannot see it.
func TestPollAttributesByFinalRecipient(t *testing.T) {
	orgA, orgB := uuid.New(), uuid.New()
	store := newMultiOrgStore(
		Address{ID: uuid.New(), OrganizationID: orgA, Address: "acme@qerds.localhost", IsDefault: true},
		Address{ID: uuid.New(), OrganizationID: orgB, Address: "globex@qerds.localhost"},
	...
	got, err := svc.Poll(context.Background(), orgA)
	if err != nil {
		t.Fatalf("Poll: %v", err)
	}
	if got != 0 {
		t.Errorf("received = %d, want 0 — the message belongs to another org", got)
	}
	if len(consumer.calls) != 1 {
		t.Fatalf("consumer calls = %d, want 1", len(consumer.calls))
	}
	if consumer.calls[0].OrgID != orgB {
		t.Errorf("message filed under the polling org, not its finalRecipient owner")
	}
}

// A recipient no org owns must not be filed under the polling org, and must not
// fail the console request either: the messages that DID arrive would be hidden
// behind a 500.
func TestPollSkipsUnknownRecipientWithoutFailing(t *testing.T) {
	orgA := uuid.New()
	store := newMultiOrgStore(
		Address{ID: uuid.New(), OrganizationID: orgA, Address: "acme@qerds.localhost", IsDefault: true},
	)
	...
	if got != 1 {
		t.Errorf("received = %d, want 1 — the org's own message must still arrive", got)
	}
	if len(consumer.calls) != 1 || consumer.calls[0].Body != "offer for acme" {
		t.Errorf("consumer calls = %+v, want only the org's own message", consumer.calls)
	}
}

// gateProvider records the high-water mark of drains inside Fetch at once. It
// returns no messages, so only the fetch half is exercised — enough to see an
// overlap, and it keeps the non-concurrent fakes out of the race.
...
func (p *gateProvider) Send(context.Context, qerdsprovider.OutboundMessage) (qerdsprovider.SendReceipt, error) {
	return qerdsprovider.SendReceipt{}, nil
}

func (p *gateProvider) ResolveAddress(_ context.Context, id string) (qerdsprovider.Address, error) {
	return qerdsprovider.Address(id), nil
}

func (p *gateProvider) Fetch(context.Context, qerdsprovider.Address) ([]qerdsprovider.InboundMessage, error) {
	p.mu.Lock()
	p.inside++
	...
	// Long enough that an unserialised pair reliably overlaps.
...
// The console's Poll and the background sweep read the same access-point queue,
// and retrieveMessage consumes: if they overlap, both can see one message id and
// the loser's retrieve fails — which on the console path is a 500 on "check
// inbox". They must never drain at the same time within a process.
func TestPollAndPollAllDoNotDrainConcurrently(t *testing.T) {
	orgA := uuid.New()
	store := newMultiOrgStore(
		Address{ID: uuid.New(), OrganizationID: orgA, Address: "acme@qerds.localhost", IsDefault: true},
	)
	...
	for range 5 {
		...
		go func() {
			defer wg.Done()
			if _, err := svc.Poll(context.Background(), orgA); err != nil {
				t.Errorf("Poll: %v", err)
			}
		}()
		go func() {
	...
	if prov.maxSeen != 1 {
		t.Errorf("%d drains were inside Fetch at once, want 1 — the drains are not serialised", prov.maxSeen)
	}
}

// A drain queued behind a long one must give up with the caller's deadline
// rather than park the goroutine past it — the console poll is a request.
func TestPollRespectsContextWhileWaitingForTheDrainSlot(t *testing.T) {
	orgA := uuid.New()
	store := newMultiOrgStore(
		Address{ID: uuid.New(), OrganizationID: orgA, Address: "acme@qerds.localhost", IsDefault: true},
	)
	svc := NewService(store, store, &gateProvider{})

	// Hold the slot, so the poll below can only wait for it.
	release, err := svc.acquireDrain(context.Background())
	if err != nil {
		t.Fatalf("acquireDrain: %v", err)
	}
	defer release()

	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Millisecond)
	defer cancel()
	if _, err := svc.Poll(ctx, orgA); !errors.Is(err, context.DeadlineExceeded) {
