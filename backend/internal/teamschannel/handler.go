...
// testSender posts the specimen card (implemented by *Channel).
type testSender interface {
	SendTest(ctx context.Context, orgID uuid.UUID, orgName string) error
}

// Handler serves an org's Microsoft Teams settings. Both reads and writes are
// org-admin only: this is who-gets-told-what configuration, and even the read says
// whether a webhook is in place.
type Handler struct {
	store       settingsStore
	channel     testSender
...
func (h *Handler) Register(mux *http.ServeMux) {
	...
	// The path segment is the channel's own id (notifications.ChannelTeams), so the
	// route and the subscription document name the channel the same way.
	mux.Handle("GET /orgs/{slug}/msteams/settings", admin(respond.HandlerFunc(h.getSettings)))
	mux.Handle("PUT /orgs/{slug}/msteams/settings", admin(respond.HandlerFunc(h.putSettings)))
	mux.Handle("POST /orgs/{slug}/msteams/test", admin(respond.HandlerFunc(h.sendTest)))
}
...
func (h *Handler) getSettings(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		return fmt.Errorf("getting microsoft teams settings: %w", err)
	}
	respond.JSON(w, r, http.StatusOK, settings)
	return nil
}

// settingsRequest is an upsert of the Teams configuration. WebhookURL is a pointer
...
func (h *Handler) putSettings(w http.ResponseWriter, r *http.Request) error {
	...
	if errors.Is(err, ErrNoEncryptionKey) {
		// A deployment without a Teams key cannot hold the secret at all, and the
		...
		return apiError(http.StatusConflict, "no_encryption_key",
			"this deployment has no Microsoft Teams encryption key, so a webhook URL cannot be stored")
	} else if err != nil {
		return fmt.Errorf("updating microsoft teams settings: %w", err)
	}
	respond.JSON(w, r, http.StatusOK, settings)
	return nil
}
	...
	if req.WebhookURL != nil {
		...
		if err != nil {
			...
			return SettingsInput{}, badRequest("invalid_input",
				"webhookUrl must be an https URL at "+WebhookHostsDescription())
		}
...
func (h *Handler) sendTest(w http.ResponseWriter, r *http.Request) error {
	...
	case errors.Is(err, ErrNotConfigured):
		return apiError(http.StatusConflict, "not_configured",
			"Microsoft Teams is not configured or is switched off for this organization")
	case errors.As(err, &delivery):
		// Teams (or the network) refused it. The admin is the one who can fix the
	...
	case err != nil:
		return fmt.Errorf("sending microsoft teams test notification: %w", err)
	}
