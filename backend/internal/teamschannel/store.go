...
// Store persists per-org Teams settings. The webhook URL is encrypted at rest
// with the deployment Teams key (`cipher`); when cipher is nil the deployment has
// no Teams key and storing a webhook URL is refused rather than kept in the clear.
type Store struct {
	db     database.DB
...
// GetSettings returns the non-secret view of an org's Teams settings.
func (s *Store) GetSettings(ctx context.Context, orgID uuid.UUID) (Settings, error) {
	const query = `SELECT webhook_url_ciphertext IS NOT NULL, enabled, updated_at
		FROM org_teams_settings WHERE organization_id = $1`
	var out Settings
	...
	if err != nil {
		return Settings{}, fmt.Errorf("teamschannel: get settings org %s: %w", orgID, err)
	}
	out.Configured = true
	return out, nil
}

// Upsert creates or updates an org's Teams settings and audits the change in the
...
// when a URL is offered and the deployment has no Teams key.
func (s *Store) Upsert(ctx context.Context, orgID uuid.UUID, in SettingsInput) (Settings, error) {
	...
	if in.WebhookURL != nil {
		...
		if *in.WebhookURL != "" {
			...
			if err != nil {
				return Settings{}, fmt.Errorf("teamschannel: encrypt webhook org %s: %w", orgID, err)
			}
	...
	err := database.InTx(ctx, s.db, func(q database.Querier) error {
		...
		const upsert = `INSERT INTO org_teams_settings
			...
			ON CONFLICT (organization_id) DO UPDATE SET
				webhook_url_ciphertext = CASE WHEN $4 THEN EXCLUDED.webhook_url_ciphertext
				                              ELSE org_teams_settings.webhook_url_ciphertext END,
				enabled = EXCLUDED.enabled,
				updated_at = now()`
		if _, err := q.Exec(ctx, upsert, orgID, webhookArg, after.enabled, setWebhook); err != nil {
			return fmt.Errorf("teamschannel: upsert settings org %s: %w", orgID, err)
		}
		...
		return s.audit.Record(ctx, q, audit.TeamsSettingsUpdated,
			audit.Target{Type: audit.TargetTeamsSettings, ID: orgID.String(), OrgID: &orgID},
...
// state is the pair of facts about an org's Teams settings that the audit event
// carries.
type state struct {
	hasWebhook bool
	enabled    bool
...
func lockedState(ctx context.Context, q database.Querier, orgID uuid.UUID) (state, error) {
	const query = `SELECT webhook_url_ciphertext IS NOT NULL, enabled
		FROM org_teams_settings WHERE organization_id = $1 FOR UPDATE`
	var current state
	switch err := q.QueryRow(ctx, query, orgID).Scan(&current.hasWebhook, &current.enabled); {
	case errors.Is(err, pgx.ErrNoRows):
		return state{}, nil
	case err != nil:
		return state{}, fmt.Errorf("teamschannel: read settings org %s: %w", orgID, err)
	}
	return current, nil
}
...
// Teams off — none of which is a failure, they are all "do not post".
func (s *Store) webhookFor(ctx context.Context, orgID uuid.UUID) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("teamschannel: webhook for org %s: %w", orgID, err)
	}
	...
	if s.cipher == nil {
		...
		return "", fmt.Errorf("teamschannel: org %s has a stored webhook but %w", orgID, ErrNoEncryptionKey)
	}
	plaintext, err := s.cipher.Decrypt(ciphertext)
	if err != nil {
		return "", fmt.Errorf("teamschannel: decrypt webhook org %s: %w", orgID, err)
	}
