...
// The tests live in the package rather than beside it because webhookFor — the read
// the channel delivers through — is deliberately unexported: the decrypted URL never
// leaves this package.
...
// testWebhook is a Power Automate trigger URL, so the round-trip covers the kind
// whose credential sits in the query rather than the path: a store that dropped or
// re-encoded the query would still pass with a connector URL.
const testWebhook = "https://prod-27.westeurope.logic.azure.com/workflows/9f8e/triggers/manual/paths/invoke?api-version=2016-06-01&sig=s3cr3tsignature"
...
// The row must never hold the plaintext URL: whoever reads the table would otherwise
// be able to post as the tenant's integration.
func TestUpsertStoresTheWebhookEncrypted(t *testing.T) {
	...
	err = pool.QueryRow(ctx,
		`SELECT webhook_url_ciphertext FROM org_teams_settings WHERE organization_id = $1`, orgID).
		Scan(&stored)
	if err != nil {
		t.Fatalf("read the stored ciphertext: %v", err)
	}
	for _, plaintext := range []string{"logic.azure.com", "s3cr3tsignature"} {
		if strings.Contains(string(stored), plaintext) {
			t.Errorf("the stored value carries the plaintext webhook url (found %q)", plaintext)
		}
	}
...
// A nil URL is "keep the stored one", which is what lets an admin switch delivery off
// and on again without pasting the webhook a second time.
...
// Delivery on with no webhook is a state the settings screen cannot render back: GET
// would answer enabled with hasWebhook false, which it shows as switched off. Only
// the API can ask for it, and the row is what both sides read, so it is clamped here
// (see nextState).
...
// Without a deployment key the secret cannot be held at all, so the save is refused
...
func TestUpsertWithoutAnEncryptionKeyIsRefused(t *testing.T) {
	...
	if err := pool.QueryRow(context.Background(),
		`SELECT count(*) FROM org_teams_settings WHERE organization_id = $1`, orgID).Scan(&rows); err != nil {
...
// A row sealed under a key the deployment no longer has cannot be recovered, and a
// silent skip would read as "this org never configured Teams". It is stated instead.
func TestWebhookForWithoutTheKeyThatSealedItIsAnError(t *testing.T) {
	pool, _ := testdb.Fresh(t)
	orgID := makeOrg(t, pool, "acme")
	ctx := context.Background()

	webhook := testWebhook
	if _, err := newTestStore(t, pool, audit.NopRecorder{}).
		Upsert(ctx, orgID, SettingsInput{WebhookURL: &webhook, Enabled: true}); err != nil {
		t.Fatalf("Upsert: %v", err)
	}

	keyless := NewStore(pool, audit.NopRecorder{}, nil)
	_, err := keyless.webhookFor(ctx, orgID)
	if !errors.Is(err, ErrNoEncryptionKey) {
		t.Errorf("webhookFor = %v, want ErrNoEncryptionKey", err)
	}
	if errors.Is(err, ErrNotConfigured) {
		t.Error("webhookFor reported a lost key as an unconfigured org")
	}
}

// The change is audited like every other settings change — and the audit event,
// which every org admin can read and export, carries whether a webhook is set rather
// than the URL itself.
func TestUpsertAuditsTheChangeWithoutTheURL(t *testing.T) {
	...
	if action != audit.TeamsSettingsUpdated || targetType != audit.TargetTeamsSettings {
		t.Errorf("audited %s/%s, want %s/%s", action, targetType,
			audit.TeamsSettingsUpdated, audit.TargetTeamsSettings)
	}
	for _, plaintext := range []string{"logic.azure.com", "s3cr3tsignature"} {
		if strings.Contains(metadata, plaintext) {
			t.Errorf("metadata = %s, want no webhook url in it (found %q)", metadata, plaintext)
		}
	}
