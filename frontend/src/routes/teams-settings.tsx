...
import {
  useSendTeamsTestMutation,
  useTeamsSettingsQuery,
  useUpdateTeamsSettingsMutation,
} from "../api/teams.queries";
import type { TeamsSettings } from "../api/teams";
...
// The backend's own message for a rejected webhook (the status it read), or the
...
function saveError(error: Error, t: TFunction): string {
  if (
    ...
    return t("teamsSettings.noEncryptionKey");
  }
  return t("teamsSettings.saveError", { message: apiMessage(error) });
}

function testError(error: Error, t: TFunction): string {
  if (error instanceof ApiError && error.status === CONFLICT_STATUS) {
    return t("teamsSettings.testNotConfigured");
  }
  if (error instanceof ApiError && error.status === BAD_GATEWAY_STATUS) {
    return t("teamsSettings.testRefused", { message: apiMessage(error) });
  }
  return t("teamsSettings.testError", { message: error.message });
}
...
function WebhookForm({
  ...
  const save = useUpdateTeamsSettingsMutation(slug);
    ...
    <Card className="max-w-2xl p-7">
      ...
      <form onSubmit={handleSave} className="mt-4 flex flex-col gap-3">
        <div className="flex flex-col gap-1">
          <label htmlFor="msteams-webhook-url" className={LABEL}>
            {t("teamsSettings.webhookUrl")}
          </label>
          <Input
            ...
            placeholder={
              initial.hasWebhook
                ? t("teamsSettings.webhookUnchanged")
                : t("teamsSettings.webhookPlaceholder")
            }
            autoComplete="off"
            spellCheck={false}
          />
          <p className="text-ink-soft text-[12px]">
            {initial.hasWebhook
              ? t("teamsSettings.webhookStored")
              : t("teamsSettings.webhookHint")}
          </p>
        </div>
        ...
        >
          ...
          />
          {t("teamsSettings.enabled")}
        </label>
        {!canEnable && (
          <p className="text-ink-soft text-[12px]">
            {t("teamsSettings.enabledNeedsWebhook")}
          </p>
        )}
...
function TestCard({
  ...
  const { t } = useTranslation();
  const test = useSendTeamsTestMutation(slug);

  return (
    <Card className="max-w-2xl p-7">
      <h2 className="text-[16px] font-semibold">
        {t("teamsSettings.testHeading")}
      </h2>
      <p className="text-ink-soft mt-1 text-[13px]">
        {t("teamsSettings.testDescription")}
      </p>
      <div className="mt-4">
        <Button
          ...
          onClick={() => test.mutate()}
        >
          {t("teamsSettings.sendTest")}
        </Button>
      </div>
      {!ready && (
        <p className="text-ink-soft mt-2 text-[12px]">
          {t("teamsSettings.testNotConfigured")}
        </p>
      )}
      {test.isError && (
...
// TeamsSettingsPanel is the org's Microsoft Teams notification configuration,
...
export function TeamsSettingsPanel({
  ...
  const { t } = useTranslation();
  const settings = useTeamsSettingsQuery(slug, true);

  if (settings.isError) {
    return (
      <Card className="p-6">
        <p className="text-error text-[14px]">
          {t("teamsSettings.loadError", { message: settings.error.message })}
        </p>
      </Card>
    );
  }
